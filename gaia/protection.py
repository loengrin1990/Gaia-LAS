...
RULES = (
    ...
    ("Идентификатор", r"\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b"),
    ("СНИЛС", r"\b\d{3}-\d{3}-\d{3}\s\d{2}\b"),
    ("ИНН", r"\b\d{10}(?:\d{2})?\b"),
    ("Документ", r"\b(?:\d{4}\s\d{6}|(?:договор|заявка)\s*№?\s*[A-Za-zА-Яа-я0-9/-]{4,})\b"),
    ("Реквизит", r"(?i)\b(?:сч[её]т|бик|карт[аы])\s*[:№]?\s*\d(?:[\d ]{7,22}\d)\b"),
