    ...
    def handle_context_get(self) -> None:
        ...
        if route.path == "/api/context/search":
            ...
            try:
                ...
                if not project.strip():
                    raise ContextSearchError("Выберите рабочее пространство для поиска.")
                intake = ControlledIntake(create_metadata=False)
                workspace_id = intake.existing_workspace(project)
                payload = search(ContextService(intake.store, workspace_id).list() if workspace_id else [], params)
