...
GAIA_RULES = [
    ...
    MaskRule(
        "PHONE",
        re.compile(
            r"\b(?:телефон|номер\s+телефона)\s*[:—-]?\s*(?:семь|восемь)"
            r"(?:[\s-]+(?:ноль|один|одна|два|две|три|четыре|пять|шесть|семь|восемь|девять|"
            ...
            r"семьдесят|восемьдесят|девяносто|сто|двести|триста|четыреста|пятьсот|шестьсот|"
            r"семьсот|восемьсот|девятьсот)){6,18}\b",
            re.IGNORECASE,
        ),
        "gaia-phone-words",
    ),
