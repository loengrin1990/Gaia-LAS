...
Этот временный режим нужен для установления фактов по двум блокирующим дефектам этапа 6: локальной проверке Veil и системному выбору файла в отдельном окне macOS. Он не меняет правила очистки, проверку координат, единый HTML upload-flow или drag-and-drop. Контракт Veil отдельно требует явного признака успешного завершения: один `findings_count=0` не является доказательством чистоты текста.
