...
В очереди допускается не более восьми ожидающих jobs сверх четырех активных.
Если лимит достигнут, `POST /api/analyze` возвращает `429 job_queue_full`.
Для multipart действуют ограничения: общий body — 25 MB, один файл — 20 MB,
до восьми файлов и до двух одновременно разбираемых uploads. При занятом
upload-лимите возвращается `503 upload_capacity_full`.
...
Завершенный job хранится в памяти ограниченное время, заданное
`processing.completed_job_retention_seconds` (по умолчанию 1800 секунд).
После очистки `GET /api/jobs/<job_id>` возвращает `404`.

## `GET /api/retention-status`

Возвращает обезличенный итог последней фактической очистки Archive: время
проверки, количество удаленных runs, journals и audit-записей, а также список
защитных пропусков. Endpoint не содержит исходные файлы, project memory или
полный prompt.

## `POST /api/jobs/<job_id>/cancel`

Отменяет подготовку асинхронного job. Ответ возвращает обновленный `JobRecord`
со `status = cancelled` и `cancellation_requested = true`.

Если job в этот момент транскрибирует медиа, Gaia завершает процесс
транскрибатора. Для остальных синхронных этапов отмена проверяется между
шагами; результат отмененной задачи не публикуется и не записывается в журнал.

Пределы задаются локально в `processing.analyze_job_timeout_seconds` и
`processing.transcription_timeout_seconds`. По истечении первого job также
получает статус `cancelled`; это не означает, что пользователь вручную нажал
отмену.

## Защита изменяющих запросов

Все `POST` и `PATCH` endpoints требуют loopback-клиента, HttpOnly SameSite
session cookie, которую Gaia выдает при открытии `/`, и совпадающий локальный
`Origin`. Если проверка не проходит, API возвращает `403
mutation_not_authorized`. Обычная работа Focus не требует дополнительных
настроек: браузер передает cookie автоматически при same-origin запросах.
