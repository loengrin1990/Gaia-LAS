...
Остальные ключевые поля: `profile_id`, `profile_title`, `prompt`, `journal_path`, `safety_audit_path`, `memory_sources`, `memory_total_sections`, `safe_for_codex_after_confirmation`, `local_fallback_required`, `policy_notes`, `query_mask_review`, `files[].mask_review`.
...
Покрытие: masking, профили, jobs, локальный статус, Project Registry, Project Dialogues, Scribe Inbox, Excel preview, memory index, prompt rebuild, Scribe, retention и UI review.
