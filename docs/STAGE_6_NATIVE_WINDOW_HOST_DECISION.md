...
## Реализация этапа 6.3

Решение реализовано в `native/macos/GaiaHost/` как Xcode macOS App project на Swift и AppKit. Bundle identifier: `io.github.loengrin1990.gaia`; Deployment Target: macOS 15.0 — это реалистичная граница для поддерживаемого AppKit/WebKit API, без искусственного обещания старых систем.

Нативный host создаёт один `NSWindow` и один `WKWebView`, использует существующий Python backend и web UI. Он не содержит бизнес-логики Gaia, не читает материалы или DOM и не создаёт отдельный upload API. File picker вызывается только нативной реализацией `WKUIDelegate`; JXA не ремонтируется и сохранён как временный диагностический fallback до ручной приёмки.

Локальная сборка: `zsh scripts/build_macos_host.sh`. Code signing, notarization и упаковка Python намеренно отложены. Поэтому распространение на другие Mac остаётся целевым требованием следующего упаковочного шага, а не обещанием готового установщика.
