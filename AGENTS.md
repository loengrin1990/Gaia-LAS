...
This project is indexed by GitNexus as **Gaia-LAS** (3894 symbols, 9070 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.

> Index stale? Run `node .gitnexus/run.cjs analyze` from the project root — it auto-selects an available runner. No `.gitnexus/run.cjs` yet? `npx gitnexus analyze` (npm 11 crash → `npm i -g gitnexus`; #1939).

## Always Do

- Apply the global **Pre-edit Impact Gate** before changing existing behaviour; use GitNexus as Gaia's project-specific code-intelligence tool for the required NORMAL or HIGH-RISK check.
...
- NEVER bypass the global Pre-edit Impact Gate when it requires a NORMAL or HIGH-RISK check.
